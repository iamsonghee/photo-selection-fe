...
function PageHeader({ right }: { right?: React.ReactNode }) {
  return (
    <header
      className="sticky top-0 z-50 flex h-12 items-center justify-between border-b border-white/10 px-4"
      style={headerBg}
    >
    ...
    return (
      <div className="min-h-screen bg-[#050505] text-zinc-100" style={gridBg}>
        ...
        <div className="flex min-h-[calc(100vh-48px)] flex-col items-center justify-center px-4 py-8">
          <div className="w-full max-w-[440px] rounded-2xl border border-white/10 bg-[#111318]/95 p-6 md:p-8" style={{ backdropFilter: "blur(8px)" }}>
            <div className="mb-6 flex justify-center">
              <div className="flex items-center gap-2">
                <Aperture className="h-6 w-6 text-[#4f7eff]" />
                <span className="text-[20px] font-bold text-zinc-100" style={playfair}>PhotoSelect</span>
              </div>
            </div>
            <h1 className="mb-1 text-center text-[22px] font-bold text-zinc-100" style={playfair}>
              보정본이 도착했습니다
            </h1>
            ...
            <div className="mb-6 space-y-3">
              {[
                ...
                <div key={i} className="flex items-center gap-3 text-[13px] text-zinc-100">
            ...
            {photographer && (
              <div className="mt-4 flex items-center gap-3 rounded-xl border border-white/10 bg-[#1a1d24] p-3">
                <img src={getProfileImageUrl(photographer.profile_image_url)} alt="" className="h-9 w-9 shrink-0 rounded-full object-cover" />
                <div>
                  <div className="text-[12px] font-semibold text-zinc-100">{photographer.name || "담당 작가"}</div>
    ...
    <div className="min-h-screen bg-[#050505] text-zinc-100" style={gridBg}>
      ...
      <div className="flex min-h-[calc(100vh-48px)] flex-col items-center justify-center px-4 py-8">
        <div className="w-full max-w-[440px] rounded-2xl border border-white/10 bg-[#111318]/95 p-6 md:p-8" style={{ backdropFilter: "blur(8px)" }}>
          ...
          <h1 className="mb-5 text-center text-[22px] font-bold leading-tight text-zinc-100" style={playfair}>
          ...
          {photographer && (
            <div className="mt-5 flex items-center gap-3 rounded-xl border border-white/10 bg-[#1a1d24] p-3">
              <img src={getProfileImageUrl(photographer.profile_image_url)} alt="" className="h-9 w-9 shrink-0 rounded-full object-cover" />
              <div>
                <div className="text-[12px] font-semibold text-zinc-100">{photographer.name || "담당 작가"}</div>
                <div className="text-[11px] text-[#5a5f78]">담당 작가</div>
