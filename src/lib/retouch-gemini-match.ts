...
export type GeminiMatchOutcome = {
  photoId: string;
  filename: string;
  similarity: number;
  type: "gemini" | "gemini_low";
};
 ...
 * Gemini 임베딩 이미지 유사도 기반 매칭을 시도한다(2026-07-30부터 OpenCLIP 대체).
 * 네트워크 실패·타임아웃 시 절대 throw하지 않고 빈 배열을 반환 — 호출 측은 항상
 * "none"(수동 지정 필요) 상태로 안전하게 폴백할 수 있다.
 */
export async function matchRetouchByGemini(
  projectId: string,
  leftoverPhotoIds: string[],
  leftoverFiles: File[],
  options?: { signal?: AbortSignal },
): Promise<GeminiMatchOutcome[]> {
  if (leftoverPhotoIds.length === 0 || leftoverFiles.length === 0) return [];

  try {
    // Gemini 임베딩 추출용이라 최종 업로드 압축보다 더 작게 줄여도 충분함
 ...
 */
export function applyGeminiMatches<T extends MappingTarget>(
  rows: MappingResult<T>[],
  leftoverFiles: File[],
  matches: GeminiMatchOutcome[],
): MappingResult<T>[] {
