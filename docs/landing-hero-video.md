...
`HeroVideo.tsx`는 중복 자동 `play()` 요청을 방지하고, `playing` 이벤트 외에도 `timeupdate`의 실제 시간 진행을 확인해 오래 남은 재생 버튼을 숨긴다. 거절된 요청은 현재 정지 상태일 때만 UI에 반영한다. 영상 소스·코덱·백엔드 API는 변경하지 않는다.
