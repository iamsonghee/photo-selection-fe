...
[작가] 파일 선택
  ...
  → 백엔드: EXIF 회전 → 썸네일(400px/75%) + 미리보기(1200px/82%)
  → R2 병렬 업로드 → DB INSERT
...
VERSION_MAX_BYTES = 2_000_000  # 2MB 상한, 초과 시 품질 자동 조정
...
|----|-----|---------|--------|---------|
| BUG-01 | CR3 조용한 실패 | **P0** | 데이터 손실 | ⬜ |
| BUG-02 | 대문자 확장자 불일치 | **P0** | 업로드 불가 | ⬜ |
| BUG-03 | 특수문자 파일명 | P1 | 접근 불가 | ⬜ |
| BUG-04 | PC 토큰 만료 | P1 | 대용량 실패 | ⬜ |
| BUG-05 | 실패 피드백 없음 | P2 | UX | ⬜ |
