...
- **시작 조건**: `/c/[token]/locked`, `status`가 `confirmed` 또는 `editing`.
- **사용자가 수행하는 단계**: `선택된 원본` 또는 `선택하지 않은 원본` 카드 클릭 → 읽기 전용 전체화면 상세보기 → 버튼·PC 방향키·모바일 스와이프로 이전/다음 사진 확인 → 닫기 또는 `ESC`로 원래 목록 복귀.
- **프론트엔드 컴포넌트**: `LockedPhotoViewer.tsx`. 셀렉을 수정하는 기존 `/viewer/[photoId]`는 `confirmed`/`editing`에서 `/locked`로 되돌리는 편집 전용 화면이므로 재사용하지 않는다.
- **성공 시 기대 결과**: 선택된 원본과 미선택 원본은 서로 섞이지 않고 각각의 섹션 안에서 순환 탐색한다. 파일명, `선택됨`/`미선택` 상태, 현재 순번을 표시하며 선택·별점·색상·코멘트 변경 UI는 노출하지 않는다. `viewerImageUrl(photo)` 기준 1200px 프리뷰(없으면 썸네일)를 사용하고 PC는 이전 1장·다음 2장, 모바일은 양옆 1장을 미리 다운로드·decode한다. 모바일 핀치·더블탭 확대 중에는 사진 이동 스와이프를 차단한다.
- **실패 및 경계 상황**: `editing_v2` 이후 검토 결과 카드는 보정본/원본 중 무엇을 상세 표시할지 별도 제품 정의가 필요하므로 이번 읽기 전용 원본 상세보기 대상에서 제외한다. 이미지 저장 방지 설정(`NEXT_PUBLIC_BLOCK_VIEWER_IMAGE_DOWNLOAD`)은 기존 고객 뷰어와 동일하게 적용한다.
- **관련 권한/인증 조건**: PIN 쿠키(미들웨어 보호 대상). 추가 API·DB 변경 없음.
- **QA에서 확인해야 할 항목**: 선택/미선택 섹션 간 탐색 범위 분리, 첫·마지막 사진 순환, 방향키·스와이프·닫기·ESC, 확대 중 오동작 여부, 모달 종료 후 목록 위치 유지.
...
- **성공 시 기대 결과**: 업로드 세션 진행 중 화면에 보이는 이미지는 서버 썸네일이 아니라 브라우저 로컬 `URL.createObjectURL()` blob URL이며(선택 직후: 원본 파일 blob, 전송 중: 압축본 blob), 세션 전체가 끝난 뒤 1회 DB를 재조회해 실제 `r2_thumb_url` 기반 그리드로 교체된다. `projects.photo_count`는 신규 논리 사진에 대해서만 증가하고 멱등 재시도는 증가시키지 않는다. 원본 포함 세션은 사진/원본 진행 수를 분리해 표시한다. 원본 PUT과 confirm은 파일마다 최초 요청을 제외한 총 4회의 공유 재시도 예산을 사용하며, 네트워크 오류와 복구 가능한 HTTP 오류에는 지수 백오프와 jitter를 적용한다. 단계별 재시도와 전체 batch 종료 후 재전송을 중첩하지 않는다. 마지막 DB 검증까지 통과한 경우에만 "업로드 완료"와 `uploaded` 로그를 기록한다. 미완료가 남으면 초대 CTA를 비활성화하고 "원본 업로드 확인 필요"와 이어 업로드 배너를 보여준다. 원본이 `pending/processing`이면 복구 오류로 오인하지 않고 버튼을 `원본 확인 중…` 상태로 유지하며 최대 15초 동안 1초 간격으로 활성화를 자동 재시도한다. `awaiting_upload`/`failed`/`NULL`처럼 실제 원본 복구가 필요한 상태가 있을 때만 복구 안내를 표시한다. `include_original=true` 업로드 시 워크플로우 화면의 원본 카드에 `original_status` 배지(`원본 처리 중`/`원본 완료`/`원본 실패`)가 표시됨. 이 "원본 처리"는 재압축이 아니라 R2에 이미 올라온 파일의 존재 확인(HEAD)뿐이라 통상 `original_compress_worker`의 다음 5초 폴링 주기 안에 끝난다. 단, Railway가 Sleep한 상태였다면 다음 HTTP 요청이 올 때까지 지연될 수 있다.
...
- **사용자가 수행하는 단계**: 결과 페이지에서 확정된 사진 목록·고객 코멘트 확인 → 필요 시 CSV/TXT로 내보내기 또는 파일명 클립보드 복사. 워크플로우의 `내보내기`에서 원본 포함 프로젝트는 `셀렉 원본 다운로드`로 업로드한 원본 파일(JPG면 변환 없는 원본 크기 JPG)을 받고, 원본 미포함 프로젝트는 `셀렉 프리뷰 다운로드`로 확인용 최대 1200px JPEG를 받을 수 있다.
