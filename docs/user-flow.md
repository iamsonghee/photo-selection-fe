...
- 완료 상태는 아래 가상 프로젝트 카드에도 반영된다. 처음부터 버튼은 선택·찜·별점·코멘트·검토·메시지를 초기화한다. 상하단·요금 영역의 “무료 시작하기” CTA는 클릭 시 세션을 확인하고, 비로그인 상태이면 로그인 모달을 연다. 로그인 후에는 작가 대시보드로 연결되며 기존 세션이 있으면 바로 이동한다. 샘플 스크롤은 “고객 셀렉 체험하기” 보조 링크로 제공한다. 랜딩 진입 시 인증 조회는 하지 않으며 이용 조건은 `SAMPLE_PLAN_LIMITS` 가상 수치를 표시한다. 가이드·베타 링크는 prefetch 없이 명시적 이동만 제공한다. 히어로는 휴대폰 고객 셀렉 → 사진별 요청 입력·저장 → 작가 결과 확인 → 보정본 한 장 업로드를 20초 무음 WebM/MP4로 반복 재생한다. 작가 결과는 실제 셀렉 갤러리와 사진 아래 코멘트를 사용하며 별도 오른쪽 요청 패널은 표시하지 않는다. 모바일 화면 폭에서는 전용 세로 영상과 poster를 사용한다. 움직임 축소 설정에서는 영상을 요청하지 않고 poster를 표시하며, 자동 재생 거절·미디어 오류에도 poster를 유지한다. 실제 고객 사진은 사용하지 않는다. 히어로 영상은 샘플 10장 중 3장을 선택하는 별도 시연이고 아래 체험의 13장/4장 상태와 독립적이다.
