...
- **성공 시 기대 결과**: 사진 그리드 렌더, 이미 저장된 선택/별점/코멘트가 있으면 그대로 표시. 작가가 "유사컷 분석"을 실행해 완료된 프로젝트라면, 흔들림/눈감음 의심 사진에 경고 배지(⚠, 카드 우상단)가 표시됨 — **정보성 참고용이며 선택/확정을 막지 않음**. 분석 완료 이후 고객이 갤러리를 처음 여는 시점 기준으로만 반영되고, 이미 갤러리를 열어둔 상태에서 작가가 재분석하면 새로고침 전까지 배지가 갱신되지 않음. 같은 조건(분석 완료된 프로젝트)에서는 "유사컷 대표이미지 적용" 토글도 함께 노출됨(기본 OFF). 켜면 각 그룹의 대표컷만 보이고 나머지 멤버는 숨겨지며, 대표컷의 "+N" 배지를 눌러야 나머지 멤버를 펼쳐볼 수 있음 — 이 토글은 고객이 선택 가능한 사진의 화면상 가시성에 직접 영향을 준다.
- **실패 및 경계 상황**:
