...
- 셀렉 PC 도구줄에 기존 파일명 복사를 직접 노출하며 선택 장수와 코멘트가 있는 사진 수를 표시한다. CSV/TXT/사진 내보내기의 기존 조건·처리는 동일하다.
