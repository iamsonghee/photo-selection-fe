...
| `api/photographer/projects/[id]/status` | GET, PATCH | 세션+소유권 | GET은 원본 전체/완료/처리/복구 필요 집계만 반환한다. PATCH의 `preparing→selecting`은 실제 프리뷰 사진 수를 확인하고 원본 완료 상태와 무관하게 원자적으로 활성화한다. |
