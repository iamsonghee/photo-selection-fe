...
추천 변경은 서버 요청보다 먼저 `acut:recommendation-draft:{projectId}`에 기록한다. 자동 저장은 한 번에 하나만 실행하고 저장 도중 생긴 변경을 최신 Set으로 다시 저장해 응답 순서 역전을 막는다. 서버 저장 성공 시에만 로컬 초안을 삭제하며, 실패하면 유지해 새로고침 후 자동 재시도할 수 있다. 저장이 끝나지 않은 동안에는 페이지 이탈과 셀렉 요청을 막고 `다시 저장`을 제공한다.

접힌 유사컷 대표 카드에는 전체 사진 배열에서 미리 계산한 그룹별 추천 개수를 전달해 `A 추천 N장`을 표시한다. 대표 사진의 추천 상태와 그룹 추천 현황을 섞지 않으며, 그룹을 펼치면 그룹 단위 표시는 숨기고 각 카드의 추천 배지만 사용한다.
...
|---|---|---|
| `photographers` | `id, auth_id, email, name, profile_image_url, bio, instagram_url, portfolio_url, contact_phone, default_selection_deadline_days, default_include_original, default_upload_strategy, created_at, beta_status("not_invited"\|"active"\|"ended"\|"suspended"), beta_start_date, beta_end_date, admin_note, total_projects_created` | `auth_id`는 Supabase Auth의 `user.id`. `default_selection_deadline_days`(NULL 또는 1~365), `default_include_original`, `default_upload_strategy`(`preview_first`/`parallel`, 초기 `parallel`)는 작가 설정의 프로젝트 기본값이며 새 프로젝트 생성 시에만 복사된다(`20260918140000_add_project_defaults.sql`, `20260919090000_optional_default_selection_deadline.sql`). 마감 기본 기간이 비어 있으면 프로젝트의 기본 7일을 사용하며, 기존 프로젝트는 설정 변경의 영향을 받지 않는다. |
| `projects` | `id, photographer_id, name, customer_name, shoot_date, deadline, selection_deadline_days, required_count, photo_count, status, access_token, access_pin, confirmed_at, delivered_at, customer_cancel_count, max_revision_count, revision_round, review_deadline, shoot_type, customer_phone, clip_analysis_status, display_id, include_original, upload_strategy, original_archive_status, original_download_started_at, original_archive_processing_started_at, cover_photo_id, cover_focal_y, created_at, updated_at` | `selection_deadline_days`는 셀렉 요청 모달의 기본 기간이고 `upload_strategy`는 `preview_first`(프리뷰 전체 등록 후 원본 시작·프리뷰 완료 시 요청 허용) 또는 `parallel`(프리뷰 등록 직후 해당 원본 전송·전체 원본 완료 후 요청 허용)이다. 기존 프로젝트는 현재 동작 보존을 위해 `preview_first`로 백필된다. 나머지 상태·아카이브 필드는 기존과 동일하며 아카이브 생성 흐름은 `user-flow.md` §8.2 참고. |
...
| `/photographer/projects/[id]/upload` | `upload/page.tsx` | 초대 전(`preparing`) 사진 업로드/편집 화면. 셀렉용 프리뷰가 모두 등록되면 전달용 원본 전송이 남아 있어도 `셀렉 요청하기`로 `preparing→selecting` 전환과 공유가 가능하다. 활성화 뒤 사진 추가·삭제는 잠기지만 현재 원본 큐는 계속 전송된다. 실패 원본이 있으면 프로젝트 작업 카드의 복구 CTA가 `?recover=1`로 이 화면을 제한적으로 다시 열며, 신규 사진 편집 없이 기존 job만 복구한다. 그 외 활성 프로젝트 재진입은 `/assets/original`로 이동한다. |
...
| `api/photographer/profile` | GET/PATCH | 세션 | 작가 프로필과 새 프로젝트 기본값(셀렉 기간·원본 포함·업로드 방식) 조회/저장 |
| `api/photographer/account` | DELETE | 세션 | 계정 삭제(통계 익명화 후 Auth 사용자 삭제) |
| `api/photographer/projects` | POST | 세션 | 프로젝트 생성 — 등급별 한도 검증 후 작가 기본값을 프로젝트 행에 복사해 INSERT |
| `api/photographer/projects/[id]` | PATCH/DELETE | 세션+소유권 | 프로젝트 기본 정보(`location` 포함)·갤러리 설정·상태 수정/삭제(+FastAPI R2 정리 호출) |
| `api/photographer/projects/[id]/status` | GET, PATCH | 세션+소유권 | GET은 원본 전체/완료/처리/복구 필요 집계만 반환한다. PATCH의 `preparing→selecting`은 실제 프리뷰 사진 수를 확인하고, `preview_first`는 원본 진행 중에도 활성화하며 `parallel`은 모든 원본 완료 후 활성화한다. |
...
| POST | `/api/upload/versions/delivery/presign` | Supabase JWT | 원본 크기 보정본 direct PUT URL 발급(파일당 최대 100MiB) |
| POST | `/api/upload/versions` | Supabase JWT | 원본 크기 파일 HEAD 검증 + 검토용 1200px/82%·300px/75% JPEG 생성 및 연결 |
...
| 납품용 원본/ZIP 및 작가 셀렉 원본 다운로드 presign | Next API 라우트(서버, `api/c/original-download/archive`, `api/c/original-download/files`, `api/photographer/projects/[id]/selected-originals`) → FastAPI | `POST /api/storage/presign`(`dispositions` 포함). 고객 상태 폴링 `api/c/original-download`는 FastAPI를 호출하지 않음. 작가 셀렉 원본은 `src/lib/directory-download-client.ts`를 고객 PC 개별 원본과 공유해 파일별 스트리밍 저장 |
...
4. **백엔드 처리(`photo-selection-be/app/routers/upload.py`)** — `POST /api/upload/photos`는 아래를 **전부 동기로 끝내야 응답**한다(즉 이 처리 시간은 FE progress bar에 반영되지 않고, 완료 여부만 이 응답으로 판가름남):
   ...
   - **`include_original=true`일 때 추가 흐름**:
     ...
     - 재시도 정책: 브라우저는 개별 PUT에 최대 3회(초기+2회)를 시도하고, 세션의 다른 batch가 모두 끝난 뒤 실패 항목만 동시 2개로 한 차례 더 전송한다. 그래도 실패하면 `/originals/report-failure`로 단계를 기록한다. Worker의 R2 source 404는 즉시 `failed`, R2 HEAD/DB 오류는 linear backoff(`attempts=1`→+5분, `attempts=2`→+30분, `max_attempts=3`).
...
6. **보정본(V1/V2) 업로드**: 브라우저가 `/api/upload/versions/delivery/presign`을 받아 원본을 `versions/{project_id}/delivery/v{version}/...`에 동시 3개(`DIRECT_UPLOAD_CONCURRENCY`)씩 direct PUT한다. 이후 `/api/upload/versions`가 HEAD 검증 후 고유한 업로드 토큰을 포함한 key에 검토용 1200px/82%와 300px/75% JPEG를 만든다. `replace_photo_versions_with_history` RPC는 기존 활성 파일과 검토 결과를 `photo_version_revisions`에 먼저 보존하고 현재 `photo_versions`를 원자적으로 교체한다. 고객 확정(`version_reviews.status='approved'`) 사진은 presign·업로드 확정·RPC에서 교체를 거부한다. 그 외 교체는 프로젝트 상태, V1/V2 단계, 재보정 허용 횟수를 변경하지 않으며 고객 API는 계속 현재 활성 행만 읽는다. 작가 상세 API만 교체 이력을 읽기 전용으로 함께 반환한다.
   - **파일-사진 매칭 우선순위**(`src/lib/version-mapping.ts`, `src/lib/retouch-gemini-match.ts`, `UploadVersionsPanel.tsx`의 `runGeminiMatchPass`): ① `exact`(확장자 제외 파일명 완전 일치) → ② `fuzzy`(편집 툴 접미사·1~2자리 버전 번호 제거 후 stem 일치, 3자리 이상 원본 순번 보존) → ③ `gemini`/`gemini_low`(Gemini 이미지 임베딩 유사도, `GEMINI_MATCH_AUTO_THRESHOLD` 기본 0.96, `GEMINI_MATCH_LOW_THRESHOLD` 기본 0.85) → ④ `order`(잔여 타깃과 파일을 순서대로 짝짓는 최후 폴백). Gemini가 0.96 이상이어도 같은 파일의 1·2위 후보 점수 차이가 `GEMINI_MATCH_MARGIN_THRESHOLD` 기본 0.03 미만이면 연결과 업로드는 유지한 채 `gemini_low`의 "AI 확인 필요"로 표시한다. 이 판정은 이미 계산한 유사도 행렬만 사용해 추가 AI 호출이나 전송을 만들지 않는다. `order`도 별도의 "순서 확인 필요" 배지로 표시되어 작가가 재지정할 수 있다.
