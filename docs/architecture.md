...
> **정정(2026-07-31)**: 과거 이 표에 있던 `POST /api/upload/originals`(납품 파일 업로드, `delivered` 상태 전용, `delivery_files` 테이블 INSERT)는 실제로는 존재하지 않는 phantom 엔드포인트였다 — 코드베이스 전체 grep으로 확인. `delivery_files` 테이블은 2026-07-23 `DROP TABLE`로 완전히 제거됐고, 납품용 원본은 `include_original` 아키텍처(프로젝트 생성 시점에 셀렉용 이미지와 함께 업로드)로 대체됐다. §7.4/§9 참고.
